...
import {
  ...
  QueryStringUtils,
} from '@openid/appauth/built/query_string_utils';
...
export class ViewControllerRequestHandler extends AuthorizationRequestHandler {
  ...
  public performAuthorizationRequest(
    ...
    this.authorizationPromise = new Promise<AuthorizationRequestResponse>(
      (resolve, reject) => {
        Agent.open(this.buildRequestUrl(configuration, request)).then(
          resultUrl => {
            const queryString = parse(resultUrl.split('?')[1]);
            const responseParams = (queryString as any) as AuthorizationResponseJson &
              AuthorizationErrorJson;
